...
#define OPTLIST_INIT(OPTL) \
	...
	OPTLIST_SET_DSP(OPTL, 'a',	"displaydata",	OPT_DSP_DISPLAYDATA,	PO_NOARG, 0,	prs_dsp_displayopt, dsp_data); \
	OPTLIST_SET_DSP(OPTL, 'A',	"simpledisplay",OPT_DSP_SIMPLEDISPLAY,	PO_NOARG, 0,	prs_dsp_displayopt, dsp_simple); \
	OPTLIST_SET_DSP(OPTL, 'v',	"verbose",	OPT_DSP_DISPLAYHEADER,	PO_NOARG, 0,	prs_dsp_displayopt, dsp_header); \
	OPTLIST_SET_DSP(OPTL, 'n',	"displaypackets",OPT_DSP_DISPLAYPACKETS,	PO_NOARG, 0,	prs_dsp_displayopt, dsp_packets); \
	OPTLIST_SET_DSP(OPTL, 'H',	"hexa",		OPT_DSP_DISPLAYHEXA,	PO_NOARG, 0,	prs_dsp_displayopt, dsp_hexa); \
	...
	OPTLIST_SET_DSP(OPTL, 'c',	"displayprotodatalink",	OPT_DSP_DISPLAYDLPROTO,	PO_NOARG, 0,	prs_dsp_displayopt, dsp_dlproto); \
	OPTLIST_SET_DSP(OPTL, 'b',	"displayprototransport",OPT_DSP_DISPLAYTLPROTO,	PO_NOARG, 0,	prs_dsp_displayopt, dsp_tlproto); \
	OPTLIST_SET_DSP(OPTL, 'B',	"displayprotonetwork",	OPT_DSP_DISPLAYNLPROTO,	PO_NOARG, 0,	prs_dsp_displayopt, dsp_nlproto); \
	OPTLIST_SET_DSP(OPTL, 'N',	"displayallpackets",	OPT_DSP_DISPLAYALLPACKETS,		PO_NOARG, 0,	prs_dsp_displayopt, dsp_allpackets); \
	OPTLIST_SET_DSP(OPTL, 'z',	"dontdisplayemptysp",	OPT_DSP_DONTDISPLAYEMPTYSLP,	PO_NOARG, 0,	prs_dsp_dontdisplayemptyslp, 0 ); \
 \
	OPTLIST_SET_FLT(OPTL, 'd',	"destport",	OPT_FLT_DSTPORT,	PO_ARG, chk_flt_dstport,	prs_flt_dstport,	flt_tl_port_dst, sizeof(__u16)); \
	OPTLIST_SET_FLT(OPTL, 'D',	"destip",	OPT_FLT_DSTIP,		PO_ARG, chk_flt_dstip,		prs_flt_dstip,		flt_nl_ip_dst, sizeof(__u32)); \
	OPTLIST_SET_FLT(OPTL, 's',	"sourceport",	OPT_FLT_SRCPORT,	PO_ARG, chk_flt_srcport,	prs_flt_srcport,	flt_tl_port_src, sizeof(__u32)); \
	OPTLIST_SET_FLT(OPTL, 'S',	"sourceip",	OPT_FLT_SRCIP,		PO_ARG, chk_flt_srcip,		prs_flt_srcip,		flt_nl_ip_src, sizeof(__u32)); \
	OPTLIST_SET_FLT(OPTL, 'f',	"filter",	OPT_FLT_FILTERSTR,	PO_ARG, chk_flt_filterstr,	prs_flt_filterstr,	flt_string, sizeof(char *)); \
	OPTLIST_SET_FLT(OPTL, 'Q',	"protocol",	OPT_FLT_PROTOCOL,	PO_ARG, chk_flt_protocol,	prs_flt_protocol,	flt_dl_protocol, sizeof(unsigned int)); \
	OPTLIST_SET_FLT(OPTL, 'p',	"ipprotocol", 	OPT_FLT_IPPROTOCOL,	PO_ARG, chk_flt_ipprotocol,	prs_flt_ipprotocol,	flt_tl_protocol, sizeof(unsigned int)); \
	OPTLIST_SET_FLT(OPTL, 'P',	"ethprotocol",	OPT_FLT_ETHPROTOCOL,	PO_ARG, chk_flt_ethprotocol,	prs_flt_ethprotocol,	flt_nl_protocol, sizeof(unsigned int)); \
	OPTLIST_SET_FLT(OPTL, 'F',	"filter-regex",	OPT_FLT_FILTERREGEX,	PO_ARG, chk_flt_filterregex,	prs_flt_filterregex,	flt_regex, sizeof(regex_t)); \
	OPTLIST_SET_FLT(OPTL, 'm',	"sourcemac",	OPT_FLT_SRCMAC,		PO_ARG, chk_flt_srcmac,		prs_flt_srcmac,		flt_dl_mac_src, 6*sizeof(__u8)); \
	OPTLIST_SET_FLT(OPTL, 'M',	"destmac",	OPT_FLT_DSTMAC,		PO_ARG, chk_flt_dstmac,		prs_flt_dstmac,		flt_dl_mac_dst, 6*sizeof(__u8)); \
	OPTLIST_SET_FLT(OPTL, 'l',	"limitnb",	OPT_FLT_LIMITNB,	PO_ARG, chk_flt_limitnb,	prs_flt_limitnb,	0, 0 ); \
	OPTLIST_SET_FLT(OPTL, 'g',	"globalport",	OPT_FLT_GLOBALPORT,	PO_ARG, chk_flt_globalport,	prs_flt_globalport,	flt_tl_port_global, sizeof(__u16)); \
	OPTLIST_SET_FLT(OPTL, 'G',	"globalip",	OPT_FLT_GLOBALIP,	PO_ARG, chk_flt_globalip,	prs_flt_globalip,	flt_nl_ip_global, sizeof(__u32)); \
...
#define PRS_DSP_LKD_ADD(L,O) \
	linked_list_add(L, linked_list_dsp_value_init(O->u.dsp.func_dsp));
...
{
	PRS_DSP_LKD_ADD(list_display,optl);
	return OPT_OK;
}
...
{
	PRS_FLT_LKD_ADD(list_filter,optl,val);
	return OPT_OK;
}
